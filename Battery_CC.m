...
if abs(Desired_Current)> Battery.IMaxRatio * C
    error('HIGH CURRENT value!')
end
%% prepare online plotting
close all
figure
subplot(2,1,1)
title('Constant Current Process')
...
disp(strcat('A', {' '}, Direction, ' CC process STARTED at:', {' '}, datestr(Main.TStart)))
...
Main.TEnd = datetime;
disp(strcat(Direction, ' CC process FINISHED at:', {' '}, datestr(Main.TEnd)))
